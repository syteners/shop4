...
# Возвращение чата (убираем дубликат)
@router.message(F.text == '💬 Чат Duplicate')
async def user_chat_old(message: Message, bot: Bot, state: FSM, arSession: ARS):
    from tgbot.data.config import get_chat_url
    from tgbot.keyboards.inline_user import chat_link_finl
    ...
    await message.answer(
        "<b>💬 Присоединяйтесь к нашему чату!</b>\n"
        "➖➖➖➖➖➖➖➖➖➖\n"
        "📱 Нажмите кнопку ниже для перехода в чат",
        reply_markup=chat_link_finl(display_url)
    )
...
async def user_buy_stars(message: Message, bot: Bot, state: FSM, arSession: ARS):
    ...
    if get_settings.status_stars_buy == "False":
        return await message.answer("<b>❗ Покупка звезд временно недоступна.</b>")
